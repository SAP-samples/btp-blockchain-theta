...
app.get("/trustee/deploy-contract-ln", async function (req, res) {
    ...
    //const account = await sbprov.getAccount(address);
...
app.get("/trustee/interact-contract-ln", async function (req, res) {
	...
	// https://docs.thetatoken.org/docs/theta-js-sdk-contract
	const contractAddress = '0xba30579024448b979b6ee2f58d30d4e814de2f7e';
	// When copying from Remix IDE, transform with this.
    // cat ABI | tr -d '[:space:]' > ABI.abi
...
app.get("/trustee/create-zombie", async function (req, res) {
    ...
    // https://docs.thetatoken.org/docs/theta-js-sdk-contract
    // Assume the zombie.sol contract has been deployed.
	const contractAddress = '0xba30579024448b979b6ee2f58d30d4e814de2f7e';
	// When copying from Remix IDE, transform with this.
    // cat ABI | tr -d '[:space:]' > ABI.abi
    ...
    responseStr += "<pre>\n";
     ...
     if (req.query && req.query.name) {
        // Create a zombie named Andrew
        const contract_result = await contract.createRandomZombie(req.query.name);
        responseStr += "contract_result :" + JSON.stringify(contract_result,null,2) + "\n";
    }
...
app.get("/trustee/get-zombie", async function (req, res) {
    ...
    if (req.query && req.query.index) {
        // Get a zombie at index offset
        const contract_result = await contract.zombies(req.query.index);
        responseStr += "contract_result :" + JSON.stringify(contract_result,null,2) + "\n";
    }
